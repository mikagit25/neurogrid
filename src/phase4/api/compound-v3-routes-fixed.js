/**
 * Compound V3 API Routes - FIXED VERSION
...
// Helper function to get compound manager
function getCompoundManager(req) {
    const phase4Manager = req.app.get('phase4Manager');
    return phase4Manager?.protocolManagers.get('compound_v3')?.manager;
}
...
router.post('/supply', async (req, res) => {
    try {
        ...
        const compoundManager = getCompoundManager(req);
        if (!compoundManager) {
